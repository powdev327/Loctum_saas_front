...
export function ClientAddManager({ isOpen, openModal, closeModal, institution }) {
  ...
  /*  institution_id: Yup.string()
        ...
        .matches(uuidRegex, "Invalid UUID format")
        .test("institution-exists", "Institution not found", (value) =>
            institutions.some((inst) => inst.institution_id.toLowerCase() === value?.toLowerCase())
        ),*/
  });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      formData.append("institution_id", values.institution_id.institution_id);
  ...
  const shouldShowError = (fieldName) => {
    return (
        ...
        (formik.submitCount > 0 && formik.errors[fieldName]) ||
        (formik.values[fieldName] && formik.errors[fieldName])
    );
  };
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div className="col-span-2">
                  ...
                  <Input
                      ...
                      type="text"
                      name="institution_id"
                      value={formik.values.institution_id.institution_id}
                      onChange={formik.handleChange}
                      disabled={true}
                      onBlur={formik.handleBlur}
                      placeholder="e.g., 123e4567-e89b-12d3-a456-426614174000"
                  />
                  {institution?.id.institution_name && !shouldShowError("institution_id") && (
                    ...
                    <div className="invisible absolute z-10 right-full top-1/2 mr-2 w-64 -translate-y-1/2 opacity-0 transition-opacity duration-300 group-hover:visible group-hover:opacity-100">
                      <div className="relative bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          If Self Billing is enabled, a new Super Client will be created under the same Client ID managing the account.
                        </p>
