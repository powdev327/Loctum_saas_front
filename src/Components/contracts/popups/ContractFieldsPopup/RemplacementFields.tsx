...
export const RemplacementFieldsComponent = ({ 
    ...
    return (
        <div className="space-y-5">
            ...
            {/* Show fields when Mission Spécialisée is selected */}
            {isMissionSpecial && (
                <>
                    {/* Hidden fields - still processed in the backend but not displayed */}
                    <div style={{ display: 'none' }}>
                    ...
                    {/* Specialty Required - This should be visible for specialized missions */}
                    <div>
                        ...
                        <Select
                            options={isDental ? [
                                ...
                                { value: "preparation_sterile", label: "Préparation stérile" },
                                { value: "oncologie", label: "Oncologie" },
                                { value: "soins_palliatifs", label: "Soins palliatifs" },
                                { value: "geriatrie", label: "Gériatrie" },
                                { value: "pediatrie", label: "Pédiatrie" }
                            ] : []}
                            placeholder="Sélectionner une spécialité"
                            value={remplacementFields.required_specialty ? 
                                { value: remplacementFields.required_specialty, label: remplacementFields.required_specialty } : 
                                null
                            }
                            onChange={(option) => setRemplacementFields({
                                ...remplacementFields,
                                required_specialty: option?.value || ""
                            })}
                        />
                    </div>
                    
                    {/* We can reuse most of the same fields from general mission */}
                    {/* Poste recherché */}
                    <div>
                        <Label required>Poste recherché</Label>
                        {submissionAttempted && !remplacementFields.position_title && (
                        ...
                        <Select
                            options={isDental ? [
                                ...
                                { value: "hygieniste_specialise", label: "Hygiéniste spécialisé" }
                            ] : isPharmacy ? [
                                { value: "pharmacien_specialiste", label: "Pharmacien spécialiste" },
                    ...
                    {/* Description détaillée des tâches specialized */}
                    <div>
                        <Label required>Description détaillée des tâches spécialisées</Label>
                        <TextArea
                    ...
                    {/* Equipment or operating room - specialized field */}
                    <div>
                    ...
                    <div>
                        <Label>Informations additionnelles</Label>
                        <TextArea
                            placeholder="Information supplémentaire pour cette mission spécialisée..."
                            value={remplacementFields.additional_information || ""}
                            onChange={(e) => {
                                const value = e && typeof e === 'object' && 'target' in e ? e.target.value : e;
                                setRemplacementFields({
