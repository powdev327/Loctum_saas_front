...
const getDashboardSubItems = (userType: "client" | "locum") => {
  if (userType === "client") {
    return [{ name: "Ecommerce", path: "/dashboard", pro: false }];
  } else if (userType === "locum") {
    return [{ name: "Analytics", path: "/analytics", pro: false }];
  }
  return [];
};
  ...
  const navItems: NavItem[] = [
    {
      icon: <GridIcon />,
      name: "Dashboard",
      subItems: getDashboardSubItems(user.user_type),
    },
   /* {
      icon: <CalenderIcon />,
      name: "Calendar",
