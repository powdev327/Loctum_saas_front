...
export default function ClientUserProfile() {
    ...
    const buildInstitutionPayload = async (data) => {
        if (!(data instanceof FormData)) {
            toast.error(t("client_profile.error.invalid_form_data"));
            throw new Error(t("client_profile.error.invalid_form_data"));
        }
        ...
        try {
            ...
            toast.error(t("client_profile.error.failed_save_institution"));
    ...
    return (
        <>
            <PageMeta
                    title={t("client_profile.pageMeta.profile_dashboard_title")}
                    description={t("client_profile.pageMeta.profile_dashboard_description")}
            />
            <PageBreadcrumb pageTitle="Profile" />
            <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
                <h3 className="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90 lg:mb-7">{t("client_profile.profile.manage_your_account")}</h3>
